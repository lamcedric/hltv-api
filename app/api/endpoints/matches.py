...
def _run_historical_scrape(
    ...
    storage = CSVStorage(data_dir=output_dir)
...
@router.post(
    ...
    description="Triggers background scraping of historical matches.",
)
def start_historical_scrape(
    ...
    output_dir: str = Query(default="./data", description="Output directory"),
    delay: float = Query(default=2.0, ge=0.5, description="Delay between requests (seconds)"),
    batch_size: int = Query(default=100, ge=10, description="Batch size for saving"),
):
    """
...
@router.get(
    ...
    description="Get statistics about stored match data.",
)
def get_storage_stats(data_dir: str = Query(default="./data", description="Data directory")):
    """
    Get statistics about the CSV storage.
...
@router.get(
    "/export/csv",
    summary="Get CSV file paths",
    description="Get paths to exported CSV files.",
)
def get_csv_exports(data_dir: str = Query(default="./data", description="Data directory")):
    """
    Get the paths to CSV data files.
    ...
    """
    storage = CSVStorage(data_dir=data_dir)
    return storage.export_to_csv(data_dir)
...
def start_incremental_update(
    background_tasks: BackgroundTasks,
    data_dir: str = Query(default="./data", description="Data directory"),
